  ...
  if(file.exists(paste0(opt$output,'/', gene_i,'/SBayesR/',gene_i,'.SBayesR.parRes'))){
    ...
    if(par_res_file_i$p < 0.01 & !is.na(par_res_file_i$p)){
      ...
      for(mod in unlist(strsplit(opt$models, ','))){
        ...
        #####
        if(mod == 'top1'){
          wgt.matrix[,colnames(wgt.matrix) == 'top1']<-ss_gene_i$BETA
        }
        ...
        if(mod == 'dbslmm'){
          ...
          system(paste0(opt$rscript,' ',opt$dbslmm,'/DBSLMM.R --plink ',opt$plink,' --block ',opt$ld_blocks,'/fourier_ls-chr',chr_i,'.bed --dbslmm ',opt$dbslmm,'/dbslmm --h2 ',par_res_file_i$hsq,' --ref ',opt$output,'/', gene_i,'/ref/ref_chr',chr_i,' --summary ',opt$output,'/', gene_i,'/DBSLMM/',gene_i,'.DBSLMM.txt --n ',round(GWAS_N,0),' --nsnp ',nsnp,' --outPath ',opt$output,'/', gene_i,'/DBSLMM/ --thread 1'))
        ...
        if(mod == 'ldpred2'){
          ...
          # Run LDPred2-auto
          multi_auto <- snp_ldpred2_auto(corr, ss_gene_i_ldpred2, h2_init = par_res_file_i$hsq,
