...
sum(smr_eqtlgen$ensembl_gene_id[smr_eqtlgen$p_SMR.BONF < 0.05] %in% twas_topsig$ensembl_gene_id[twas_topsig$TWAS.P.BONF < 0.05]) # 217
sum(smr_eqtlgen$ensembl_gene_id[smr_eqtlgen$p_SMR.BONF < 0.05 & smr_eqtlgen$p_HEIDI > 0.05] %in% twas_topsig$ensembl_gene_id[twas_topsig$TWAS.P.BONF < 0.05 & twas_topsig$COLOC.PP4 > 0.8]) # 214
...
# Read in TWAS results
twas_files<-list.files(path=paste0('/users/k1806347/oliverpainfel/biomarkers-brc-mh/TWAS_resource/eQTL_to_TWAS/data/SCZ/twas_OLD_03092022//'), pattern=paste0('SCZ.TWAS.eQTLGen.chr'))

twas<-NULL
for(i in twas_files){
  twas<-rbind(twas, fread(paste0('/users/k1806347/oliverpainfel/biomarkers-brc-mh/TWAS_resource/eQTL_to_TWAS/data/SCZ/twas_OLD_03092022/',i)))
}
...
pseudo_files<-list.files(path=paste0('/users/k1806347/oliverpainfel/biomarkers-brc-mh/TWAS_resource/eQTL_to_TWAS/data/pseudo_full_OLD_03092022/'), pattern=paste0('pseudo_res_chr'))
...
mad_omnibus<-NULL
for(i in 1:22){
  mad_omnibus<-rbind(mad_omnibus, read.table(paste0('/users/k1806347/oliverpainfel/biomarkers-brc-mh/TWAS_resource/eQTL_to_TWAS/data/SCZ/twas_OLD_03092022/SCZ.TWAS.eQTLGen.omnibus_edit.chr',i,'.omnibus.pv'), header=T, stringsAsFactors = F))

}

mad_omnibus$BHHR.P.BONF<-p.adjust(mad_omnibus$BHHR.P, method='bonferroni')
mad_omnibus[mad_omnibus$GENE == 'ENSG00000100058',]
mad_omnibus[mad_omnibus$GENE == 'ENSG00000121940',]
mad_omnibus[mad_omnibus$GENE == 'ENSG00000151665',]

# The BHHR aggreagate test finds ENSG00000100058 to be highly significant, but ENSG00000121940 and ENSG00000151665 to be non-significant after bonferroni. This suggest the aggregate test might also help reduce false positives. Lets see what the simulations say.

```

```{r, eval=F, echo=T}
##########
# Compare significant and colocalised results found by ACAT.O but not in SMR
##########
...
twas_eqtl$P_onesided<-twas_eqtl$TWAS.P/2
twas_eqtl$P_onesided[twas_eqtl$TWAS.Z < 0]<- 1-twas_eqtl$P_onesided[twas_eqtl$TWAS.Z < 0]

# Calc ACAT-O pvalue
twas_eqtl_gene<-data.table(ID=sort(unique(twas_eqtl$ID)),
                                     ACATO.P=aggregate(twas_eqtl$TWAS.P, list(twas_eqtl$ID), FUN=ACATO)$x,
                                     ACATO.P_one=aggregate(twas_eqtl$P_onesided, list(twas_eqtl$ID), FUN=ACATO)$x,
...
res$P_onesided<-res$P/2
res$P_onesided[res$Z < 0]<- 1-res$P_onesided[res$Z < 0]

fpr_acat_omni_all<-NULL
fpr_acat_omni_one_all<-NULL
for(i in unique(res$perm)){
  res_i<-res[res$perm == i,]
  res_i$Gene<-gsub('.RDat.*','',gsub('eQTLGen.eQTL.','',res_i$ID))
  ...
  twas_gene<-data.table(ensembl_gene_id=unique(twas$Gene),
                                     ACATO.P=aggregate(res_i$P, list(res_i$Gene), FUN=ACATO)$x,
                                     ACATO.P_one=aggregate(res_i$P_onesided, list(res_i$Gene), FUN=ACATO)$x)
  ...
  fpr_acat_omni_one_all<-c(fpr_acat_omni_one_all, sum(twas_gene$ACATO.P_two < 0.05)/sum(!is.na(twas_gene$ACATO.P_two)))

}

mean(fpr_acat_omni_all) # 0.05397758
sd(fpr_acat_omni_all) # 0.00279165
mean(fpr_acat_omni_one_all) # 0.05506942
sd(fpr_acat_omni_one_all) # 0.002803079
