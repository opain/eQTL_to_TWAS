...
option_list = list(
  ...
  make_option("--bhhr", action="store_true", default=FALSE,
              help="Perform the BHHR omnibus test for genes (ID field) with multiple models (FILE field). NOTE: This disables all other tests."),
...
if (any(opt$omnibus,opt$bhhr)) {
	...
	if ( is.na(opt$omnibus_corr) ) cat( "GENE\tNUM.REF\tMIN.TWAS.P\tNUM.REF.PRUNED\tOMNIBUS.P\tBHHR.Z\tBHHR.P\n" , file=paste(opt$out , ".omnibus.pv" , sep=''))
				
	for ( g in dup.genes ) {
		cur.keep = which( wgtlist$ID == g )
		M = length( cur.keep )
		ge_g.matrix = matrix(nrow=nrow(genos$bed),ncol=M)
		ref.names = paste0(unlist(lapply(strsplit(dirname(wgtlist$FILE[ cur.keep ]),"/"),tail,1)),'_',wgtlist$MODEL[ cur.keep ])
				...
				if ( sum(!pruned) > 1 ) {
				  if(opt$omnibus){
					...
					if(opt$bhhr){
  					...
  					agg_res<-agg(id=ID, es=z, var=se, cor=abs(ge_g.cor[!pruned,!pruned]), method="BHHR", mod=NULL, data=data.frame(ID='A',
  					                                                                                                         z=ge_z[!pruned],
  					                                                                                                         se=1))
					...
					write.table( cbind(g,format(cur.tbl,digits=3)) , quote =F , col.names=F , row.names=F , sep='\t' , file=paste( opt$out , ".CORR.dat" , sep='') , append=T )
					cat( g , M , min( cur.wgt$TWAS.P ) , sum(!pruned) , pv.chi , agg_Z, agg_P, '\n' , sep='\t' , file=paste(opt$out , ".omnibus.pv" , sep='') , append=T )
				}
			}
		}
	}
