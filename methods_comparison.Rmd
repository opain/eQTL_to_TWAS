...
cut -f 1 -d' ' /scratch/groups/biomarkers-brc-mh/TWAS_resource/eQTL_to_TWAS/data/eQTLGen/eQTLGen_sumstats_exclGTeX.txt | tail -n +2 | uniq > /scratch/groups/biomarkers-brc-mh/TWAS_resource/eQTL_to_TWAS/data/eQTLGen2/gene_list.txt
...
## Comparison of pseudovalidation approaches

Here we will try to predict the best eQTL-based model. We will test two approaches:

- FUSION pseudovalidation - uses the TWAS method to test for an association between the SNP-weights and the original eQTL summary statistics. A high TWAS.Z would indicate the SNP-weights correspond to the eQTL sumstats well.
- MegaPRS - MegaPRS, implemented within LDAK, aims to identify the best PRS model without a validation sample. It performs well to other PRS pseudovalidation methods.

***

### FUSION pseudovalidation
