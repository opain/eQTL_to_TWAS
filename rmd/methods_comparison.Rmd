...
Rscript -e 'remotes::install_github("yaowuliu/ACAT@e737d7d188ba6731800c5e6971539ca6a3888405")'
...
This was originally run by Zac Gerring, but I have since obtained access to GTEx v8 WGS sequence data via a dbGAP application with Alan Hodgkinson.

***

#### Convert WGS data into PLINK1 binary format

We are doing this for running both OTTERS (to replicate their original analysis), and for our use later for evaluating gene expression models.

<details><summary>Show code</summary>

```{bash}
# Split the vcf by chromosome, convert to plink1 and retain only SNPs with MAF > 0.001
mkdir -p /users/k1806347/oliverpainfel/Data/dbGAP/GTEx/processed
for chr in $(seq 1 22); do
  sbatch -p neurohack_cpu,cpu -t 600 --mem 20G --wrap="
    /users/k1806347/oliverpainfel/Software/plink2 \
      --vcf /users/k1806347/oliverpainfel/Data/dbGAP/GTEx/GTEx_Analysis_2017-06-05_v8_WholeGenomeSeq_866Indiv.vcf.gz \
      --chr ${chr} \
      --make-bed \
      --snps-only \
      --maf 0.001 \
      --threads 1 \
      --out /users/k1806347/oliverpainfel/Data/dbGAP/GTEx/processed/gtex_v8.snp.maf001.chr${chr}
    "
done
```
...
#### Extract HapMap3 SNPs and insert RSIDs

Use GenoPred format_target.R for this.
...
for chr in $(seq 1 22); do
  sbatch -p neurohack_cpu,cpu -t 20 --mem 10G --wrap="
    Rscript ../Scripts/format_target/format_target.R \
            --target /users/k1806347/oliverpainfel/Data/dbGAP/GTEx/processed/gtex_v8.snp.maf001.chr${chr} \
            --format plink1 \
            --ref resources/data/ref/ref.chr${chr} \
            --output /users/k1806347/oliverpainfel/Data/dbGAP/GTEx/processed/gtex_v8.snp.maf001.hm3.chr${chr}
    "
done

# Convert the output files from plink2 to plink1 format
for chr in $(seq 1 22); do
  sbatch -p neurohack_cpu,cpu -t 20 --mem 10G --wrap="
    /users/k1806347/oliverpainfel/Software/plink2 \
      --pfile /users/k1806347/oliverpainfel/Data/dbGAP/GTEx/processed/gtex_v8.snp.maf001.hm3.chr${chr} \
      --make-bed \
      --threads 1 \
      --out /users/k1806347/oliverpainfel/Data/dbGAP/GTEx/processed/gtex_v8.snp.maf001.hm3.chr${chr}
    "
done
...
Rscript ../Scripts/Ancestry_identifier/Ancestry_identifier.R \
      --target_plink_chr /users/k1806347/oliverpainfel/Data/dbGAP/GTEx/processed/gtex_v8.snp.maf001.hm3.chr \
      --ref_plink_chr resources/data/ref/ref.chr \
      --output /users/k1806347/oliverpainfel/Data/dbGAP/GTEx/ancestry/gtex_v8.ancestry \
...
```{bash}
mkdir -p /scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp
conda activate feature_pred
sbatch -p neurohack_cpu,cpu --mem 30G -n 5 -t 200 --wrap="
  Rscript /users/k1806347/oliverpainfel/Software/MyGit/Predicting-TWAS-features/FeaturePred.V2.0.R \
  	--PLINK_prefix_chr /users/k1806347/oliverpainfel/Data/dbGAP/GTEx/processed/gtex_v8.snp.maf001.hm3.chr \
  	--ref_ld_chr /users/k1806347/oliverpainfel/Data/1KG/FUSION/1000G.EUR. \
  	--ref_maf /users/k1806347/oliverpainfel/Data/1KG/FUSION/1000G.EUR. \
  	--weights /users/k1806347/oliverpainfel/Analyses/eQTL_to_TWAS/data/YFS/FUSION/YFS.BLOOD.RNAARR.pos \
  	--weights_dir /users/k1806347/oliverpainfel/Analyses/eQTL_to_TWAS/data/YFS/FUSION \
  	...
  	--all_mod T \
  	--pigz /users/k1806347/oliverpainfel/Software/pigz \
  	--output /scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/YFS.BLOOD.RNAARR
"
```
...
sbatch -p neurohack_cpu,cpu --mem 30G -n 5 -t 200 --wrap="
  Rscript /users/k1806347/oliverpainfel/Software/MyGit/Predicting-TWAS-features/FeaturePred.V2.0.R \
  	...
  	--all_mod T \
  	--pigz /users/k1806347/oliverpainfel/Software/pigz \
  	--output /scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/YFS.eQTL
"
...
sbatch -p neurohack_cpu,cpu --mem 30G -n 5 -t 200 --wrap="
  Rscript /users/k1806347/oliverpainfel/Software/MyGit/Predicting-TWAS-features/FeaturePred.V2.0.R \
  	...
  	--ref_ld_chr /users/k1806347/oliverpainfel/Data/1KG/FUSION/1000G.EUR. \
  	--ref_maf /users/k1806347/oliverpainfel/Data/1KG/FUSION/1000G.EUR. \
  	--weights /users/k1806347/oliverpainfel/Analyses/eQTL_to_TWAS/data/eQTLGen/weights/eQTLGen.eQTL.pos \
  	--weights_dir /users/k1806347/oliverpainfel/Analyses/eQTL_to_TWAS/data/eQTLGen/weights \
  	--plink /users/k1806347/oliverpainfel/Software/plink1.9.sh \
  	--n_cores 5 \
  	--save_score T \
  	--save_ref_expr T \
  	...
  	--all_mod T \
  	--pigz /users/k1806347/oliverpainfel/Software/pigz \
  	--output /scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/eQTLGen
"
...
# Restrict the data to EUR
# Use threshold of 0.9, as it returns number similar to original GTEx paper
pop_prep<-fread('/users/k1806347/oliverpainfel/Data/dbGAP/GTEx/ancestry/gtex_v8.ancestry.model_pred', header=T)
pop_prep_eur<-pop_prep[pop_prep$EUR > 0.9]
obs<-obs[,names(obs) %in% c(names(obs)[1:4], pop_prep_eur$FID), with=F]
...
library(biomaRt)
ensembl = useEnsembl(biomart="ensembl", dataset="hsapiens_gene_ensembl", GRCh=37)
ensembl <- useEnsembl(biomart="ensembl", dataset="hsapiens_gene_ensembl", GRCh=37 , host="useast.ensembl.org")
...
eqtl<-fread('/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/YFS.eQTL/FeaturePredictions_YFS.eQTL.txt.gz')

names(eqtl)<-gsub('.RDat','',gsub('YFS.','', names(eqtl)))

fusion<-fread('/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/YFS.BLOOD.RNAARR/FeaturePredictions_YFS.BLOOD.RNAARR.txt.gz')
...
saveRDS(cor_res, '/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/pred_obs_cor_YFS_full.RDS')
...
fwrite(cor_res_melt, '/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/pred_obs_cor_YFS_full.txt', quote=F, sep=' ', na='NA')
write.table(cor_res_melt_obs, '/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/pred_obs_cor_YFS.txt', row.names=F, quote=F)

# cor_res_melt<-fread('/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/pred_obs_cor_YFS_full.txt')
# cor_res_melt_obs<-fread('/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/pred_obs_cor_YFS.txt')
...
library(ggplot2)
bitmap('/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/pred_obs_cor_boxplot.png', units='px', width=1500, height=1000, res=300)
...
bitmap('/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/pred_obs_cor_pairsplot.png', units='px', width=5000, height=5000, res=300)
...
write.csv(n_valid, '/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/n_valid.csv', row.names=F)
...
fwrite(cor_res_melt_eqtl_i_obs_diff_all_count_tab, '/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/yfs_method_better_than_other.csv', quote=T, na='NA')
...
fwrite(cor_res_melt_eqtl_i_obs_diff_all_sig_best, '/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/yfs_method_better_than_all.csv', quote=T, na='NA')
...
fwrite(cor_res_melt_eqtl_i_obs_diff_all_sig_eqtl_worse, '/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/yfs_eqtl_worst_than_best.csv', quote=T, na='NA')
...
![](/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/pred_obs_cor_boxplot.png)

![](/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/pred_obs_cor_pairsplot.png)

```{r, eval=T, echo=F}
n_valid<-fread('/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/n_valid.csv')
...
# Restrict the data to EUR
# Use threshold of 0.9, as it returns number (714) similar to original GTEx paper
pop_prep<-fread('/users/k1806347/oliverpainfel/Data/dbGAP/GTEx/ancestry/gtex_v8.ancestry.model_pred', header=T)
pop_prep_eur<-pop_prep[pop_prep$EUR > 0.9]
obs<-obs[,names(obs) %in% c(names(obs)[1:4], pop_prep_eur$FID), with=F]
...
# Read in predicted expression
eqtl<-fread('/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/eQTLGen/FeaturePredictions_eQTLGen.eQTL.txt.gz')
...
fwrite(cor_res_melt, '/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/pred_obs_cor_full.txt', sep=' ', quote=F, na='NA')

write.table(cor_res_melt_obs, '/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/pred_obs_cor.txt', row.names=F, quote=F)

# cor_res_melt<-fread('/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/pred_obs_cor_full.txt')
# cor_res_melt_obs<-fread('/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/pred_obs_cor.txt')
...
library(ggplot2)
bitmap('/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/pred_obs_cor_boxplot_eQTLGen.png', units='px', width=1500, height=1000, res=300)
ggplot(cor_res_melt_obs, aes(x=paste0(Var1,' vs. ',Var2), y=value)) +
  geom_boxplot() +
  labs(x='Test', y='Correlation') +
  coord_flip()
...
bitmap('/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/pred_obs_cor_pairsplot_eQTLGen.png', units='px', width=3000, height=3000, res=300)
ggpairs(cor_res_melt_obs_unmelt[,-1], lower = list(continuous = wrap(lowerfun)), diag = list(continuous = wrap(diagfun)))
dev.off()
...
write.csv(n_valid, '/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/n_valid_eQTLGen.csv', row.names=F)
...
fwrite(cor_res_melt_i_obs_diff_all_count_tab, '/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/eqtlgen_method_better_than_other.csv', quote=T, na='NA')
...
fwrite(cor_res_melt_i_obs_diff_all_sig_best, '/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/eqtlgen_method_better_than_all.csv', quote=T, na='NA')
...
fwrite(cor_res_melt_i_obs_diff_all_sig_eqtl_worse, '/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/eqtlgen_eqtl_worst_than_best.csv', quote=T, na='NA')
...
![](/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/pred_obs_cor_boxplot_eQTLGen.png)

![](/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/pred_obs_cor_pairsplot_eQTLGen.png)

```{r, eval=T, echo=F}
n_valid<-fread('/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/n_valid_eQTLGen.csv')
kable(n_valid, rownames = FALSE)
...
eqtlgen<-fread('/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/pred_obs_cor.txt')
yfs<-fread('/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/pred_obs_cor_YFS.txt')
...
png('/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/pred_obs_cor_eQTLGen_YFS_comp.png', units='px', width=1500, height=1500, res=300)
...
png('/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/present_in_YFS_hist.png', units='px', width=1500, height=1000, res=300)
...
png('/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/present_in_YFS_hist_sigsig.png', units='px', width=1500, height=1000, res=300)
...
![](/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/pred_obs_cor_eQTLGen_YFS_comp.png)

![](/scratch/prj/oliverpainfel/Analyses/eQTL_to_TWAS/data/GTEx_v8/pred_exp/present_in_YFS_hist.png)
...
for (j in c('P0.001')) {
  ...
  res_1kg <- rbind(res_1kg, tmp)
}

# Perform ACAT.O analysis
res_1kg<-res_1kg[complete.cases(res_1kg),]
res_1kg_acat<-data.table(aggregate(res_1kg$FUSION_PVAL, list(res_1kg$TargetID), FUN=ACAT))
names(res_1kg_acat)<-c('TargetID', 'OTTERS')

library(reshape2)
res_1kg_wide <- dcast(res_1kg, TargetID ~ MODEL, value.var = "FUSION_PVAL")
res_1kg_wide<-merge(res_1kg_wide, res_1kg_acat, by = 'TargetID')

write.csv(res_1kg_wide, '/users/k1806347/oliverpainfel/Analyses/eQTL_to_TWAS/data/OTTERS/rep_res_1kg.csv', row.names=F)
...
for (j in c('P0.001')) {
  ...
  tmp <- tmp[, c('TargetID', 'MODEL', 'FUSION_Z', 'FUSION_PVAL','FUSION_PVAL_GC'), with = F]
...
write.csv(res_gtex_wide, '/users/k1806347/oliverpainfel/Analyses/eQTL_to_TWAS/data/OTTERS/rep_res_gtex.csv', row.names=F)
...
both<-Reduce(function(dtf1, dtf2) merge(dtf1, dtf2, by = "TargetID"), list(orig,res_1kg_wide,res_gtex_wide))
...
both_z<-data.table(both_z)
both_z[order(both_z$OTTERS.x),]
both_z[order(both_z$OTTERS.y),]

# Check I can reproduce the original OTTERS p-value using the ACAT function
orig_long<-melt(orig)
names(orig_long)<-c('TargetID','MODEL','FUSION_PVAL')
orig_long<-orig_long[orig_long$MODEL != 'OTTERS',]
orig_long<-orig_long[complete.cases(orig_long),]
orig_long_acat<-data.table(aggregate(orig_long$FUSION_PVAL, list(orig_long$TargetID), FUN=ACAT))
names(orig_long_acat)<-c('TargetID', 'OTTERS')

merge(orig_long_acat, orig[, c('TargetID','OTTERS'), with=F], by='TargetID')
# Yes, the OTTERS p-value is identical, so the ACAT function is not the reason for the discrepancy.
...
rep<-fread('/users/k1806347/oliverpainfel/Analyses/eQTL_to_TWAS/data/OTTERS/rep_res.csv')
...
eqt_to_twas<-eqt_to_twas[, c('ID','MODEL','TWAS.P'), with=F]
names(eqt_to_twas)[1]<-'TargetID'

# Retain features present in each set
common <-
  intersect(
    intersect(orig$TargetID, rep$TargetID), 
    eqt_to_twas$TargetID)

orig<-orig[orig$TargetID %in% common,]
rep<-rep[rep$TargetID %in% common,]
eqt_to_twas<-eqt_to_twas[eqt_to_twas$TargetID %in% common,]

# Convert eQTL_to_TWAS to wide format
# Calculate ACAT result
# Perform ACAT.O analysis
ACATO <- function(p){
...
eqt_to_twas_acat<-data.table(aggregate(eqt_to_twas$TWAS.P, list(eqt_to_twas$TargetID), FUN=ACATO))
names(eqt_to_twas_acat)<-c('TargetID', 'OTTERS')

library(reshape2)
eqt_to_twas_wide <- dcast(eqt_to_twas, TargetID ~ MODEL, value.var = "TWAS.P")
eqt_to_twas_wide<-merge(eqt_to_twas_wide, eqt_to_twas_acat, by = 'TargetID')

# Compare the ACAT P-value from each set
names(orig)[names(orig) == 'OTTERS']<-'ACAT_orig'
names(rep)[names(rep) == 'OTTERS']<-'ACAT_rep'
names(eqt_to_twas_wide)[names(eqt_to_twas_wide) == 'OTTERS']<-'ACAT_eqtl'

all<-Reduce(function(dtf1, dtf2)
  merge(dtf1, dtf2, by = "TargetID"),
  list(orig, rep, eqt_to_twas_wide))
all<-all[, c('TargetID','ACAT_orig','ACAT_rep','ACAT_eqtl'), with=F]
...
sum(all$ACAT_orig < 0.001)
sum(all$ACAT_rep < 0.001)
sum(all$ACAT_eqtl < 0.001)

table(all$ACAT_rep < 0.001, all$ACAT_eqtl < 0.001)
