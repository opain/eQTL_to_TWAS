...
In this document we explore methods for:

1) Determining which genes to derive TWAS SNP-weights for
2) Derive SNP-weights predicting gene expression

We evaluate approaches with comparison to existing TWAS models from FUSION, and observed gene expression in GTEx.
...
pop_prep<-fread('/scratch/prj/bcn_gtex_eqtl_twas/scratch_tmp/ancestry/gtex_v8.ancestry.model_pred', header=T)
...
# Remove SuSiE1 model
cor_res_melt_obs <- cor_res_melt_obs[cor_res_melt_obs$Var2 != 'eQTL.susie1',]
...
cor_res_melt <- cor_res_melt[cor_res_melt$Var2 != 'eQTL.susie1',]
cor_res_melt_obs <- cor_res_melt_obs[cor_res_melt_obs$Var2 != 'eQTL.susie1',]
...
for(i in unique(cor_res_melt$L1)){
  ...
  cor_res_melt_i_obs_3<-merge(cor_res_melt_i_obs_2, cor_res_melt_i_obs_2, by.x=c('L1','Var2'), by.y=c('L1','Var1.y'), allow.cartesian=TRUE)
...
length(unique(eqtlgen$L1[eqtlgen$value > 0.1])) # 10770
length(unique(yfs$L1[yfs$value > 0.1])) # 3329
...
nrow(both_best) # 4609
median(both_best$value.x) # 0.2173572
median(both_best$value.y) # 0.1879074
...
eqtl_pos<-fread('/users/k1806347/oliverpainfel/Analyses/eQTL_to_TWAS/data/eQTLGen/weights/eQTLGen.eQTL.pos')
w_dir<-'/users/k1806347/oliverpainfel/Analyses/eQTL_to_TWAS/data/eQTLGen/weights/'
eqtl_sig<-NULL
for(i in 1:nrow(eqtl_pos)){
  load(paste0(w_dir, eqtl_pos$WGT[i]))
  eqtl_sig<-rbind(eqtl_sig,
...
fwrite(eqtl_sig, '/users/k1806347/oliverpainfel/Analyses/eQTL_to_TWAS/data/eQTLGen/weights/eQTLGen.eQTL.sig_info.csv', quote=F, na='NA')

# eqtl_sig <-fread('/users/k1806347/oliverpainfel/Analyses/eQTL_to_TWAS/data/eQTLGen/weights/eQTLGen.eQTL.sig_info.csv')

# Restrict to genes with either FDR sig gene association or gneome-wide sig eQTL
eqtl_sig<-merge(eqtl_sig, Genes, by.x='ID', by.y='ensembl_gene_id')
eqtl_sig<-eqtl_sig[eqtl_sig$TopSNP_Pvalue < 5e-8 | eqtl_sig$P_mBATcombo < 0.01,]
nrow(eqtl_sig) # 16009

eqtlgen_comp_plot_mbat<-eqtlgen_comp_plot[eqtlgen_comp_plot$L1 %in% eqtl_sig$external_gene_name,]
...
It would be useful to be able to estimate the performance of each model, without access to individual-level training data. Here we will explore some approaches.
...
# Read in observered correlation in GTEx v8
